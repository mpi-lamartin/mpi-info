...
<Solution file={TP} lang="c" show={frontMatter.cor}></Solution>
...
6. Définir une fonction `int first_fit(instance *inst)` renvoyant le nombre de boîtes utilisées par l'algorithme `First-fit` en C.
