...
- `e` si le sommet $i$ est libre ;
- `j` et `i` si les sommets $i$ et $j$ sont appariés.

Un chemin $x_{0}, \dots, x_{k}$ sera simplement représenté par la liste `]` :
```ocaml
type path = int list
```

0. Écrire une fonction `elementaire : path -> bool` renvoyant `true` si le chemin est élémentaire (c'est-à-dire : ne passe pas deux fois par le même sommet) et `false` sinon. On fera en sorte d'avoir une complexité linéaire en le nombre $n$ de sommets du graphe.  
1. Écrire une fonction `is_augmenting` prenant en entrée un couplage \cl!m! et un chemin et indiquant si le chemin est augmentant. On supposera sans le vérifier que le chemin est élémentaire (ne passe pas deux fois par le mêmeet que les entiers sont bien entre 0 et $|m|$.
...
\item `None` si $M$ n'admet pas de chemin augmentant ;
\item `Some p`, avec $p$ un chemin augmentant, s'il en existe un.  
...
5. Écrire une fonction `maximum_matching : bipartite -> matching` renvoyant un couplage maximal pour le graphe biparti $G$.
