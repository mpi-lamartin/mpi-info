...
On pourra utiliser `strlen` de `string.h` pour connaître la taille d'une chaîne de caractères.
...
2. Écrire une fonction `add` permettant d'ajouter un élément (au début) d'une liste chaînée.  
3. Écrire une fonction `print` permettant d'afficher les éléments d'une liste chaînée.  
4. Écrire une fonction `del` permettant de supprimer l'élément du début d'une liste chaînée.  
...
## Doublons et algorithme de Floyd
...
1. Écrire un algorithme en complexité temporelle O($n$) pour résoudre ce problème. Quelle est la complexité spatiale ? 
2. Écrire un algorithme en complexité O($n$) en temps et O($1$) en mémoire. On pourra modifier $t$
...
3. Écrire un algorithme naïf pour tester si `l` contient un cycle. Quelle est sa complexité en temps et en espace ?
...
5. Écrire une fonction `bool has_cycle(linked_list l)` détectant un cycle en utilisant l'algorithme du lièvre et de la tortue.
:::