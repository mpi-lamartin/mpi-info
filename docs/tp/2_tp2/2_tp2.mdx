...
On utilisera le type suivant d'automate déterministe, où les états sont entre $0$ et $n-1$ et les lettres sont des entiers ($a$ est représenté par $0$, $b$ par $1$...) :
...
Si `a` est de type `afd`, `a.delta.(i).(j)` est l'état atteint en lisant la lettre $j$ à partir de l'état $i$ (c'est-à-dire $\delta(i, j)$). Le nombre d'états de `a` est donc `Array.length a.delta`.
...
2. Définir une fonction `delta_etoile : afd -> int -> int list -> int` telle que `delta_etoile a q u` renvoie l'état atteint en lisant le mot $u$ à partir de l'état $q$ (c'est-à-dire $\delta^*(q, u)$). Vérifier avec `a1`.

3. Définir une fonction `accepte : afd -> int list -> bool` telle que `accepte a u` détermine si $u$ est reconnu par `a`. Vérifier avec `a1`. Quelle est la complexité de `accepte` ?

4. Définir une fonction `complementaire : afd -> afd` telle que `complementaire a` renvoie un automate reconnaissant le complémentaire du langage reconnu par `a`. Vérifier avec `a1`.

5. Définir une fonction `accessibles : afd -> int list` telle que `accessibles a` renvoie la liste des états accessibles depuis l'état initial de `a`. Pour cela, on pourra utiliser un parcours en profondeur. Vérifier sur des exemples.
...
6. En déduire une fonction `vide : afd -> bool` déterminant si un automate reconnaît le langage vide.

## Automate produit

7. Écrire une fonction `inter : afd -> afd -> afd` telle que `inter a b` renvoie un automate reconnaissant l'intersection des langages reconnus par `a` et `b`. On suppose que `a` et `b` ont le même alphabet.  
...
9. En déduire une fonction `equivalent a b` déterminant si les langages reconnus par les automates `a` et `b` sont égaux. Donner la complexité.
