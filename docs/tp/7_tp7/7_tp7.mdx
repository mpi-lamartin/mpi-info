...
Pour $e$ un état (grille) et $v \in \{0, ..., 15\}$, on note $e_{v}^{i}$ la ligne de l'entier $v$ dans $e$ et $e_{v}^{j}$ sa colonne.
On pose alors :
$$
  h(e)
  = \sum_{v = 0}^{15} | e_v^i - \left\lfloor \frac{v}{4} \right\rfloor |
