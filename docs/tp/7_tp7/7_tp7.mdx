...
10. Écrire une fonction `a_star : state -> int` qui prend en entrée un état initial et renvoie la longueur d'une solution trouvée par l'algorithme $A^*$. On renverra $-1$ s'il n'y a pas de solution. Tester avec les exemples suivants.
