...
[Solution](10_tp_mnist/tp_cor.zip)

1. Télécharger [tp_apprentissage.zip](10_tp_mnist/tp_apprentissage.zip) et le charger dans le Codespace avec clic droit -> charger.
...
10. Écrire une fonction `void compute_centers(data* X, int n, int* classes, int k, data* centers)` qui met à jour les centres des classes obtenues par l'algorithme des $k$-moyennes dans le tableau `centers`, où `classes[i]` est la classe de `X[i]`, `n` est la taille de `X` et `k` est le nombre de classes.

11. Écrire une fonction `void compute_classes(data* X, int n, data* centers, int k, int* classes, bool* has_changed)` qui met à jour les classes obtenues par l'algorithme des $k$-moyennes dans le tableau `classes`, où `n` est la taille de `X` et `k` est la taille de `centers`. `has_changed` est un pointeur qui doit être mis à vrai si une donnée change de classe, faux sinon.
