...
1. Écrire une fonction `union : 'a list -> 'a list -> 'a list` renvoyant l'union de deux listes sans doublon. Par exemple, `union [1; 2] [3; 1]` peut renvoyer `[1; 2; 3]` (l'ordre des éléments de la liste de retour n'importe pas).
...
6. Écrire une fonction `p : 'a regexp -> 'a list` renvoyant l'ensemble $P(L)$ d'une expression régulière.  
7. Écrire une fonction `s : 'a regexp -> 'a list` renvoyant l'ensemble $S(L)$ d'une expression régulière.
8. Écrire une fonction `produit : 'a list -> 'b list -> ('a * 'b) list` renvoyant le produit cartésien de deux listes. Par exemple, `produit [1; 2] [3; 1]` peut renvoyer `[(1, 3); (1, 1); (2, 3); (2; 1)]` (l'ordre des éléments de la liste de retour n'importe pas).
