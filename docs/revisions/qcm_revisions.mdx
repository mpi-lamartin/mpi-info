...
<QCMRandom
  ...
  questions={[
    {
      question:
        "Quels sont les valeurs possibles de `c` si deux threads exécutent la fonction `f` ?\n```c\nint c = 10;\n\nvoid f() {\n  for(int i = 0; i < 5; i++) {\n    c--;\n  }\n}\n```",
      answers: [
        "Seulement 0",
        "De 0 à 8",
        "De 0 à 9",
    ...
    },
    {
      ...
      correct: [0, 1, 2, 4],
      explanation:
        "Complémentaire : déterminiser l'automate puis inverser états initiaux et finaux. Différence et intersection : automate produit. Miroir : construction de l'automate miroir. Inclusion : faux car tout langage contient $\\emptyset$ et est inclus dans $\\Sigma^*$.",
    },
    {
    ...
    },
    {
      ...
      correct: [0, 1, 2, 3],
      explanation:
        "Le tas (file de priorité) est utilisé par Dijkstra et A* pour extraire le sommet le plus proche, par le tri par tas, et par Huffman pour fusionner les arbres de fréquences minimales.",
    },
    {
    ...
    },
    {
      ...
      answers: [
        ...
        "Type OCaml `type 'a arb = Empty \\mid Node of 'a * 'a arb * 'a arb`",
    ...
    },
    {
      ...
      correct: [1, 3, 5],
      explanation:
        "Soit $\\varphi$ une réduction de $\\Pi_1$ à $\\Pi_2$. Si $\\Pi_2 \\in$ P, alors $\\Pi_1$ est dans P car on peut résoudre une instance $I$ de $\\Pi_1$ en résolvant $\\varphi(I)$ dans $\\Pi_2$. Si $A$ est un vérificateur pour $\\Pi_2$, alors $A(\\varphi(I), c)$ est un vérificateur pour $\\Pi_1$. Si $\\Pi_2$ est NP-complet et $\\Pi_1 \\in$ NP, alors $\\Pi_1$ est NP-complet par transitivité.",
    },
    {
    ...
    },
    {
      ...
      answers: [
        ...
        "$\\texttt{2-SAT}$",
        "$\\texttt{3-SAT}$",
        "$\\texttt{FNC-SAT}$ : $\\texttt{SAT}$ restreint aux formes normales disjonctives (FNC)",
        "Couplage maximum",
        "Clique maximum",
    ...
    },
    {
      ...
      answers: [
        ...
        "$\\lnoC(\\varphi_1 \\land \\varphi_2) \\equiv \\lnot \\varphi_1 \\lor \\lnot \\varphi_2$",
        "$\\lnoC(\\varphi_1 \\lor \\varphi_2) \\equiv \\lnot \\varphi_1 \\land \\lnot \\varphi_2$",
      ],
      correct: [0, 1, 2, 3, 4, 5],
      explanation:
    ...
    },
    {
      ...
      correct: [0, 2, 4],
      explanation:
        "$\\Pi_1 \\leq_p \\Pi_2$ signifie que $\\Pi_1$ se réduit à $\\Pi_2$, c'est-à-dire qu'il existe une fonction de réduction de $\\Pi_1$ à $\\Pi_2$.\nPar conséquent, si $\\Pi_1$ est indécidable, alors $\\Pi_2$ doit également être indécidable, car sinon on pourrait résoudre $\\Pi_1$ en utilisant la réduction vers $\\Pi_2$\n Il faut que la réduction soit polynomiale et avoir l'appartenance à NP pour conclure sur les problèmes NP-complets.",
    },
    {
      question:
        "Le lemme de l'étoile sur un langage régulier $L$ commence ainsi :",
      answers: [
        "$\\forall u \\in L, \\exists n \\in \\mathbb{N}, |u| \\geq n \\Longrightarrow \\exists x, y, z \\in \\Sigma^*, u = xyz, |xy| \\leq n, |y| > 0$",
        "$\\exists n \\in \\mathbb{N}, \\forall u \\in L, |u| \\geq n \\Longrightarrow \\exists x, y, z \\in \\Sigma^*, u = xyz, |xy| \\leq n, |y| > 0$",
        "$\\forall u \\in L, \\exists n \\in \\mathbb{N}, |u| \\geq n \\Longrightarrow \\forall x, y, z \\in \\Sigma^*, u = xyz, |xy| \\leq n, |y| > 0$",
        "$\\exists n \\in \\mathbb{N}, \\forall u \\in L, |u| \\geq n \\Longrightarrow \\forall x, y, z \\in \\Sigma^*, u = xyz, |xy| \\leq n, |y| > 0$",
      ],
      correct: 1,
      explanation:
    ...
    },
    {
      ...
      answers: [
        ...
        "Une formule $\\texttt{2-SAT}$ (FNC où chaque clause contient au plus 2 littéraux)",
        "Une formule $\\texttt{3-SAT}$ (FNC où chaque clause contient au plus 3 littéraux)",
      ],
      correct: [0, 1, 2, 4],
      explanation:
        "Toute formule est équivalente à une FNC et à une FND, en passant par une table de vérité.\nPar induction et en poussant les négations vers les variables, on obtient une formule équivalente en forme normale négative.\nOn a montré dans la réduction de $\\texttt{SAT}$ à $\\texttt{3-SAT}$ que toute formule est équivalente à une formule $\\texttt{3-SAT}$.\nPar contre, $\\texttt{2-SAT}$ est dans P, donc on aurait P = NP si toute formule était équivalente à une formule $\\texttt{2-SAT}$.",
    },
    {
      question:
    ...
    },
    {
      ...
      answers: [
        ...
        "`false` car `0.1` est stocké de façon approchée",
        "`true` car `0.1 +. 0.2` est égal à `0.3`",
      ],
      correct: 2,
      explanation:
        "Les flottants sont stockés en base 2 et `0.1` n'a pas d'écriture exacte en base 2, donc les comparaisons d'égalité stricte peuvent échouer à cause des erreurs d'arrondi.",
    },
    ...
    },
    {
      ...
      answers: [
        ...
        "La mémoïsation",
        "Accélérer directement l'algorithme des $k$ plus proches voisins",
      ],
      correct: [0, 1, 2],
      explanation:
...
correct: [0, 3, 4],
explanation:
"Un ordre topologique existe exactement pour les DAG. Il n'est pas forcément unique. Il peut être calculé par l'algorithme de Kahn (basé sur une file), en temps linéaire en sommets + arêtes.",
},
{
